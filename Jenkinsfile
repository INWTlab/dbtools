    ...
    stage('Launch mysql test database') {
      steps {
        sh '''
          ...
          docker build -t mysql-test-database -f inst/db/mysql/Dockerfile . && docker run --name mysql-test-database -p 3301:3306 -d mysql-test-database --default-authentication-plugin=mysql_native_password
          sleep 15s
